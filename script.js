...
const VideoTime = () => {
    ...
    videoRange.style.backgroundSize = (videoRange.value - videoRange.min) * 100 / (videoRange.max - videoRange.min) + '% 100%';
...
videoRange.addEventListener('mousemove', (e) => {
    let preview = document.querySelector('.preview');
    let layer = e.layerX;
    let width = videoRange.offsetWidth;
    preview.style.display = "flex";
    preview.style.left = (layer - 65) + "px";
    if (layer < 65) {
        preview.style.left = 0 + "px";
    }
    if (layer > (width - 65)) {
        preview.style.left = (width - 130) + "px";
    }
    const previewVideo = document.getElementById('previewVideo');
    previewVideo.src = videoSrc;
    var mouseX = layer - videoRange.offsetLeft;
    var progressPercent = (mouseX / width) * 100;
    var videoTime = (progressPercent / 100) * myVideo.duration;
    previewVideo.currentTime = videoTime;
    const minutes = Math.floor((videoTime % 3600) / 60);
    const seconds = Math.floor((videoTime % 3600) % 60);
});
videoRange.addEventListener('mouseleave', () => {
